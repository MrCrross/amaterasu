            ...
            @foreach ($data as $key => $indication)
                <x-table.row>
                    ...
                    <x-table.data class="w-20">
                        <x-a.info href="{{ route('contraindications.show',$contraindication->id) }}" class="mr-2">Просмотр</x-a.info>
                        @can('contraindication-edit')
                            <x-a.primary href="{{ route('contraindications.edit',$contraindication->id) }}" class="mr-2">Редактировать
                            </x-a.primary>
                        @endcan
